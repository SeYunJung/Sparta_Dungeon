...
public class Interaction : MonoBehaviour
{
    ...
    private float _lastCheckTime; // 마지막으로 체크한 시간  -> Ray가 마지막으로 아이템을 감지한 시간
    ...
    private IInteractable _curInteractable; // IInteractable 인터페이스를 구현한 무언가(ItemObject). 현재 상호작용하고 있는 아이템 
        ...
        if(Time.time - _lastCheckTime > checkRate)
        {
            ...
            if (Physics.Raycast(ray, out hit, maxCheckDistance, layerMask)) // 충돌되면 
            {
                ...
                if (hit.collider.gameObject != curInteractGameObject)
                {
                    ...
                    _curInteractable = hit.collider.GetComponent<IInteractable>(); // 충돌한 오브젝트의 아이템 정보?를 저장
    ...
    private void SetPromptText()
    {
        ...
        promptText.text = _curInteractable.GetInteractPrompt(); // ItemObject에서 아이템 이름, 설명을 반환 
    }
    ...
    public void OnInteractInput(InputAction.CallbackContext context)
    {
        ...
        if(context.phase == InputActionPhase.Started && _curInteractable != null)
        {
            ...
            _curInteractable = null; // 현재 상호작용하고 있는 아이템 비우기 
            promptText.gameObject.SetActive(false);
        }
    }
}
